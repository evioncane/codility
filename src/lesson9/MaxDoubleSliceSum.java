...
public class MaxDoubleSliceSum {
    public static void main(String[] args) {
        ...
        System.out.printf("Result is: %d", maxDoubleSliceSum.alternateSolution(A));
        System.out.println();
    }

    public int solution(int[] A) {
        int maxSlice = 0, maxEnding = 0;
        Integer smallestLocal = null;
        for (int i = 1; i < A.length - 1; i++) {
            if (smallestLocal == null || smallestLocal > A[i]) {
                smallestLocal = A[i];
            }
            maxEnding = maxEnding + A[i];
            if (maxEnding + A[i] - smallestLocal < 0) {
                maxEnding = A[i];
                smallestLocal = A[i];
            } else {
                maxSlice = Math.max(maxSlice, maxEnding - smallestLocal);
            }
        }
        return maxSlice;
    }

    public int alternateSolution(int[] A) {
